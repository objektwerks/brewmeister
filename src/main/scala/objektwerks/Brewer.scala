...
final class Brewer(listener: Listener):
  ...
  def simulate(recipe: Recipe): Batch =
    ...
    brew( Mill(recipe) )
    brew( Mash(recipe) )
    brew( LogMashingTempPh(150, 5.6) )
    brew( Lauter(recipe) )
    brew( Sparge(recipe) )
    brew( LogMashEfficiency(recipe, 4.0) )
    brew( Boil(recipe) )
    brew( Cool(recipe) )
    brew( Whirlpool(recipe) )
    brew( LogBoilingCoolingTempOriginalGravity(150, 72, 1.060) )
    brew( Ferment(recipe) )
    brew( LogFermentingTempFinalGravity(72, 1.012) )
    brew( Condition(recipe) )
