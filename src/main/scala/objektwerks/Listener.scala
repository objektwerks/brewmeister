  ...
  def batch: Batch =
    ...
    events.foreach {
      case Sanitizing(log, started) =>
        batch = batch.copy(process = batch.process.copy(sanitizingStarted = started),
          log = batch.log ++ log)
      case Sanitized(log, completed) =>
        batch = batch.copy(process = batch.process.copy(sanitizingCompleted = completed),
          log = batch.log ++ log)
      case Preparing(log, started) =>
        batch = batch.copy(process = batch.process.copy(preparingStarted = started),
          log = batch.log ++ log)
      case Prepared(recipe, style, volume, log, completed) =>
        batch = batch.copy(process = batch.process.copy(preparingCompleted = completed),
          recipe = recipe,
          style = style,
          volume = volume,
          log = batch.log ++ log)
      case Malting(log, started) =>
        batch = batch.copy(process = batch.process.copy(maltingStarted = started),
          log = batch.log ++ log)
      case Malted(log, completed) =>
        batch = batch.copy(process = batch.process.copy(maltingCompleted = completed),
          log = batch.log ++ log)
      case Milling(log, started) =>
        batch = batch.copy(process = batch.process.copy(millingStarted = started),
          log = batch.log ++ log)
      case Milled(log, completed) =>
        batch = batch.copy(process = batch.process.copy(millingCompleted = completed),
          log = batch.log ++ log)
      case Mashing(log, started) =>
        batch = batch.copy(process = batch.process.copy(mashingStarted = started),
          log = batch.log ++ log)
      case Mashed(log, completed) =>
        batch = batch.copy(process = batch.process.copy(mashingCompleted = completed),
          log = batch.log ++ log)
      ...
      case Sparging(log, started) =>
        batch = batch.copy(process = batch.process.copy(spargingStarted = started),
          log = batch.log ++ log)
      case Sparged(log, completed) =>
        batch = batch.copy(process = batch.process.copy(spargingCompleted = completed),
          log = batch.log ++ log)
      case MashEfficiencyLogged(mashEfficiency, log) =>
        batch = batch.copy(mashEfficiency = mashEfficiency,
          log = batch.log ++ log)
      case Boiling(log, started) =>
        batch = batch.copy(process = batch.process.copy(boilingStarted = started),
          log = batch.log ++ log)
      ...
      case Whirlpooling(log, started) =>
        batch = batch.copy(process = batch.process.copy(whirlpoolingStarted = started),
          log = batch.log ++ log)
      case Whirlpooled(log, completed) =>
        batch = batch.copy(process = batch.process.copy(whirlpoolingCompleted = completed),
          log = batch.log ++ log)
      ...
      case Kegging(log, started) =>
        batch = batch.copy(process = batch.process.copy(keggingStarted = started),
          log = batch.log ++ log)
      ...
      completed) =>
        batch = batch.copy(ibuBitterness = ibuBitterness,
          ...
          taste = taste,
          log = batch.log ++ log)
        batch = batch.copy(process = batch.process.copy(keggingCompleted = completed))
